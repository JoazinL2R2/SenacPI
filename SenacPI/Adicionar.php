<?php 
    ...
    $Texto = mysqli_real_escape_string($conexao, $_POST['historiaViagem']);

    $extensao = strtolower(substr($_FILES['fotoPais']['name'], -4));
    ...
    foreach ($listaPalavroes as $palavra) {
        if (stripos($Texto, $palavra) !== false) {
            echo '<script>';
            echo 'alert("Texto invalido. Ajuste e tente novamente.");';
            echo 'window.location.href = "AdicionarViagem.php";';
            echo '</script>';
            exit();
        }
    }

    if (move_uploaded_file($_FILES['fotoPais']['tmp_name'], $diretorio . $novo_nome)) {
        $sql = "INSERT INTO publicacao(Pais, Cidade, Foto, Texto) VALUES ('$Pais', '$Cidade', '$novo_nome', '$Texto')";
            
         $query = mysqli_query($conexao, $sql);
    
        if ($query) {
            echo '<script>';
            echo 'alert("Avaliação adicionada com sucesso.");';
            echo 'window.location.href = "DicasDeViagens.php";';
            echo '</script>';
        } else {
            $resposta = array('status' => 'erro', 'mensagem' => mysqli_error($conexao));
         }
    } else {
        echo '<script>';
        echo 'alert("Erro ao tentar realizar upload da imagem,tente novamente, se o erro persistir, tente outra imagem.");';
        echo 'window.location.href = "DicasDeViagens.php";';
        echo '</script>';
    }
